...
def _parse(proto):
    keys_to_features = {
        ...
        "cloth_category": tf.io.FixedLenFeature([], tf.string),
        ...
        'local_binary_pattern': tf.io.FixedLenFeature([], tf.string),
    }
    ...
    local_binary_pattern = tf.cast(tf.image.decode_png(parsed_features["local_binary_pattern"], channels=3), tf.float32) / 255.0
    ...
    data = {
        ...
        "cloth_category": parsed_features["cloth_category"],
...
def _filter(data, model_inputs):
    ...
    return tf.strings.regex_full_match(string, "stripes")
