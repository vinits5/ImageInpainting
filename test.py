...
def test():
    ...
    epoch = 30
    load_path = f"{opt.checkpoints_dir}/{epoch}_net_G.pth"
    net_G = networks.define_G(opt)
    ...
    with torch.no_grad():
        for i in range(num_iterations):
            ...
            I_i = data['I_i'].to(opt.device)
            I_g = data['I_g'].to(opt.device)
            mask = data['M'].to(opt.device)
            I_i = I_i * (1-mask)

            try:
                # I_g, I_o, I_i, val_loss_G, I_raw, L_o, mask = model.optimize_parameters(val=True)
                I_o, _ = net_G(I_i, None, mask)
                I_raw = net_G.output
            except: 
                print("Error")
                continue
    ...
    losses = {'ssim': val_ssim, 'val_mae': val_mae, 'psnr': val_psnr, 'ids': ids}
    import pandas as pd 
